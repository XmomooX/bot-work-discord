...
Client.on("guildMemberRemove", async (member) => {
    ...
    const logchannel = await db.get(`logchannel.${member.guild.id}`);
    ...
    if (kickLog) {
        ...
        if (kickLimit > 0) {
            ...
            return executor.send(`You have ${remaining} more times to kick.`);
        } else {
            try {
        ...
        if (banLimit > 0) {
            ...
            return executor.send(`You have ${remaining} more times to ban.`);
        } else {
            try {
                const removedRoles = guildMember.roles.cache
                    .map(role => role.id)
