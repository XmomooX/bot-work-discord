...
module.exports = {
    ...
    async execute(interaction, db) {
        ...
        const permissionsBackup = await db.get(
            `permissionsBackup.${interaction.guild.id}`,
        );

        if (!permissionsBackup) {
            return interaction.editReply(
                "No lockdown data found. Make sure to run the `/lockdown` command before using `/unlockdown`.",
            );
        }
        ...
        try {
                    ...
                    roles.forEach((role) => {
                        ...
                        if (permissions) {
                            channel.permissionOverwrites.edit(role, {
                                ViewChannel:
                                    permissions.includes("ViewChannel"),
                                SendMessages:
            ...
            await interaction.editReply(
                "Lockdown has been lifted. All permissions have been restored.",
            );
        } catch (error) {
            console.error("Error lifting lockdown:", error);
            await interaction.editReply(
                "An error occurred while lifting the lockdown.",
            );
        }
    },
};
